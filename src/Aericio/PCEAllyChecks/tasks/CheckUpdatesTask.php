...
namespace Aericio\PCEAllyChecks\tasks;

use Aericio\PCEAllyChecks\PCEAllyChecks;
...
class CheckUpdatesTask extends AsyncTask
{
    ...
    {
        $releases = Internet::getURL("https://poggit.pmmp.io/releases.json?name=PCEAllyChecks");
    ...
    public function onCompletion(Server $server): void
    {
        if ($this->getResult() !== null) {
            $plugin = $server->getPluginManager()->getPlugin("PCEAllyChecks");
            if ($plugin instanceof PCEAllyChecks && $plugin->isEnabled()) {
                $data = json_decode($this->getResult(), true);
                $plugin->getLogger()->info("PCEAllyChecks v" . $data[0]["version"] . " is available for download at " . $data[0]["artifact_url"] . "/PCEAllyChecks.phar");
            }
        }
    }
